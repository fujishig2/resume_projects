...
void *task_thread(void *task){
  ...
  for (i = 0; i < tuple[0].tasks[0].res_no; i++){
	...
	k++;
      ...
      //save the value of the resources that the task uses
      if (found){
	sem_i[l] = j;
  ...
  try_again:
    errno = 0;
    //try to lock the semaphore. We might want to lock the same semaphore
    //more than once due to the values given. Thus, we use a nested for loop.
    for (i = 0; i < l; i++)
	...
	if (errno == EAGAIN){
	  //if there was an error, all the currently locked semaphores
	  //must be freed.
	  for (h = 0; h < i; h++)
    ...
    //release all of the used semaphores
    for (i = 0; i < l; i++)
      for (j = 0; j < sem_val[i]; j++)
